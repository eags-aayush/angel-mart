...
export default function CheckoutPage({ currentUser, userData, cart, fullData, onOrderSuccess, deliveryInfo }) {
  ...
  const items = Object.keys(cart)
    .map(itemName => {
      ...
      return { name: itemName, qty: cart[itemName], price }
    })
    .filter(Boolean)

  const subtotal = items.reduce((sum, i) => sum + i.price * i.qty, 0)

  const displayAddress = deliveryInfo?.mode === 'delivery'
  ...
  const placeOrder = async () => {
    ...
    const templateParams = {
      ...
      // Comma-separated → EmailJS sends to both when To Email = {{to_email}}
      to_email:      `database.anglemart@gmail.com,${currentUser?.email || ''}`,
      address:       displayAddress,
      // EmailJS Mustache loop: {{#orders}}...{{/orders}}
      orders: items.map(i => ({
        name:      i.name,
      ...
      // {{cost.total}} dot-notation access
      cost: {
  ...
  if (placed) {
    return (
      <div className="section-full" style={{ textAlign: 'center', padding: '0 5%' }}>
        ...
        <p style={{ opacity: 0.6, marginBottom: 8, fontSize: 15 }}>
        ...
        <div className="checkout-card">
          ...
          <div className="log-input-group">
            <label className="log-label">
              {deliveryInfo?.mode === 'delivery' ? '🚚 DELIVERY ADDRESS' : '📍 PICKUP ADDRESS'}
            </label>
            <input
              type="text"
              className="log-field"
              ...
              return (
                <div key={item.name} className="checkout-item-row">
                  ...
                  <div className="checkout-item-meta">
                    ...
                    <div className="checkout-item-unit">
